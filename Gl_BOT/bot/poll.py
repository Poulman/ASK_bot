...
def poll(message,update,mail):
    ...
    def q_update_poll_param():
        bot.send_message(message.chat.id, en_msg_update_section, reply_markup=topic)
    ...
    def q_is_regional_center():
        bot.send_message(message.chat.id, en_msg_is_reg_centr, reply_markup=YesNo)
        bot.register_next_step_handler(message, location2)

    def q_country():
        bot.send_message(message.chat.id, en_msg_сountry_option, reply_markup=Countries)
        bot.register_next_step_handler(message, country)

    def q_citi():
        bot.send_message(message.chat.id, en_msg_location_option)
        bot.register_next_step_handler(message, city)

    def q_mobil_ter():
        bot.send_message(message.chat.id, en_msg_when)
    ...
    def q_can_work():
        bot.send_message(message.chat.id, en_q_can_work, reply_markup=YesNo)
        bot.register_next_step_handler(message, can_work)

    def q_productivity():
        bot.send_message(message.chat.id, en_q_productivity, reply_markup=percent)
        bot.register_next_step_handler(message, productivity)

    def q_can_work_reason():
        bot.send_message(message.chat.id, en_msg_reason)
        bot.register_next_step_handler(message, reason)

    def q_difficulties():
        bot.send_message(message.chat.id, en_q_diff)
        bot.register_next_step_handler(message, difficulties)

    def q_plan_relocate():
        bot.send_message(message.chat.id, en_q_location, reply_markup=YesNo)
        bot.register_next_step_handler(message, plan_relocate)

    def q_plan_relocate_info():
        bot.send_message(message.chat.id, en_msg_location)
        bot.register_next_step_handler(message, plan_relocate_info)

    def q_relocate_out():
        bot.send_message(message.chat.id, en_q_relocate, reply_markup=YesNo)
        bot.register_next_step_handler(message, relocate_out)

    def q_relocate_out_info():
        bot.send_message(message.chat.id, en_msg_relocate, reply_markup=relocate_info)
        bot.register_next_step_handler(message, relocate_out_info)

    def q_need_some():
        bot.send_message(message.chat.id, en_q_supplies, reply_markup=need_some_k)
    ...
    def q_need_some_more():
        bot.send_message(message.chat.id, en_q_supplies_more, reply_markup=need_some_k)
        bot.register_next_step_handler(message, need_some)

    def q_additional():
        bot.send_message(message.chat.id, en_q_more_help, reply_markup=YesNo)
        bot.register_next_step_handler(message, additional)

    def q_additional_info():
        bot.send_message(message.chat.id, en_msg_more_help)
        ...
        if message.text != 'Back':
            if message.text == 'Yes':
                ...
                if not update:
                    ...
                    bot.send_message(message.chat.id, en_msg_updated)
                    output(mail)
                    return
                ...
                bot.send_message(message.chat.id, en_msg_error_use_but)
                q_is_regional_center()
        else:
            location()
    ...
    def mobil(message):
        bot.send_message(message.chat.id, en_msg_mobilizade, reply_markup=mobilizade)
        ...
        if message.text != 'Back':
            ...
            if update:
                if not get_by_params_mail(mail, 'CAN_WORK'):
                    bot.send_message(message.chat.id, en_q_can_work_still)
                    q_can_work()
                else:
                    bot.send_message(message.chat.id, en_msg_updated)
                    output(mail)
    ...
    def finish_q(message):
        ...
        bot.send_message(message.chat.id, en_msg_finish_q)
...
def output(mail):
    ...
    if info[0][15] != "No":
        all_good=False
        msg+="&#9888 <b>Mobilization status:</b> {}\n".format(info[0][15])
        if not info[0][17]:
            msg+="&#9888 <b>Can't work</b>\n"
    else:
        if not info[0][17]:
            all_good=False
    ...
    if info[0][22] != "need_equipment_info":
        all_good=False
        msg+="&#9888 <b>Need equipment:</b> {}\n".format(info[0][22])
    if info[0][24] != "needresources_info":
        all_good=False
        msg+="&#9888 <b>Need resources:</b> {}\n".format(info[0][24])
    if info[0][29] != "neadsuplies_info":
        all_good=False
        msg+="&#9888 <b>Need supplies:</b> {}\n".format(info[0][29])
    if info[0][11]:
        all_good=False
        msg+="&#9888 <b>Ready to relocate</b>\n"
    if info[0][13]:
        msg+="&#9888 <b>Can move out of Ukraine in case of:</b> {}\n".format(relocate(mail))
    if info[0][19] == 'productivity':
        msg+="&#128187 <b>Productivity:</b> 0%\n"
    else:
        msg+="&#128187 <b>Productivity:</b> {}\n".format(info[0][19])
        if info[0][19] != '100%' and info[0][19] != '0%':
            all_good=False
            msg+="&#9888 <b>Difficulties:</b> {}\n".format(info[0][20])
    if info[0][32] != 'needhelp_info':
        all_good=False
        msg+="&#9888 <b>Additional information:</b> {}".format(info[0][32])
